...
Professional, comprehensive, and data-driven. Provide actionable insights with clear reasoning.`
    ...
    const context = {
      ...
      currencyPair,
      userSettings: {
        weekly_pip_target: `${profile.weekly_pip_target_min}-${profile.weekly_pip_target_max}`,
        max_risk_pips: profile.max_risk_pips_per_trade,
        pip_target_per_trade: profile.pip_target_per_rotation,
        breakeven_trigger: profile.breakeven_trigger_pips,
        trading_volume: profile.trading_volume_chf,
        leverage: profile.leverage_enabled ? profile.max_leverage : 1,
